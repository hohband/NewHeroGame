...
signal channel_interrupted(unit: Unit)
...
var collectable := false                # 关卡可夺取物件（生辰纲担等；召唤物恒为 false，决策日志 D38）
var hero: Hero = null                   # 养成档案（来自玩家阵容；技能等级等养成数值经此生效）
var channeling: Unit = null             # 正在引导夺取的目标物件（1 回合引导，受击打断，策划文档 7.3）
var alert_triggered := false            # 警觉特性：首次睡眠减免是否已触发（决策日志 D31）
...
func tick_effects() -> Array:
	...
	for b in buffs.duplicate():
		if not b.tick_effect.is_empty():
			...
			elif b.tick_effect.get("kind") == "hot" and amount > 0:
				var applied := heal(amount)
				if applied > 0:
					events.append({"type": "hot", "unit": self, "buff": b.buff_id, "amount": applied})
	return events
