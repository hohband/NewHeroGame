...
var _player_deaths := 0                 # 我方阵亡数（S 评价用，决策日志 D37）
...
## 评价（决策日志 D37）：满足全部 S 条件 = S，其余通关 = A
func _compute_rank() -> String:
	if level.rank_rules.is_empty():
		return "A"
	if bool(level.rank_rules.get("s_no_death", false)) and _player_deaths > 0:
		return "A"
	if round_count > int(level.rank_rules.get("s_max_rounds", 99)):
		return "A"
	return "S"
...
func _on_unit_died(unit: Unit) -> void:
	...
	if unit.team == Unit.Team.PLAYER and not unit.is_object:
		_player_deaths += 1
	unit_died.emit(unit)
